...
class TradingScheduler:
    ...
    def _job_next_run(self, job) -> str:
        ...
        3.11+ makes it a property that raises AttributeError while the job is
        pending (before ``scheduler.start()``). Returns 'pending' in that case.
        """
        for attr in ("next_run_times", "next_run_time"):
            try:
