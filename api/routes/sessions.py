    ...
    with run_lock:
        for r in runs.values():
            ...
            if r.session_id in seen_ids:
                continue
            meta = read_json_or_none(session_dir(r.session_id) / "meta.json") or {}
            partial = {}
            try:
...
def get_session(session_id: str):
    ...
    summary = read_json_or_none(session_dir(session_id) / "summary.json")
    if summary:
    ...
    meta = read_json_or_none(session_dir(session_id) / "meta.json")
...
def delete_session(session_id: str):
    """Delete a backtest session from the local filesystem."""
    deleted_from = []
...
def get_session_progress(session_id: str):
    ...
    progress = read_json_or_none(session_dir(session_id) / "progress.json") or {}
...
def get_session_state(session_id: str):
    ...
    return read_json_or_none(session_dir(session_id) / "agent_state.json")
...
def get_session_snapshot(session_id: str):
    ...
    snap = read_json_or_none(session_dir(session_id) / "snapshot.json")
    if snap:
        return snap
