...
Bar data is cached as local parquet files (under settings.cache_dir) to
avoid re-fetching full history on every cycle. On subsequent runs, only the
last few days are fetched incrementally.
"""
...
class LiveProvider(DataProvider):
    ...
    def __init__(self, settings) -> None:
        ...
        self._cache_dir = getattr(settings, "cache_dir", ".cache")

    def _cache_path(self, interval: str) -> str:
        return os.path.join(self._cache_dir, f"bars_{interval}.parquet")

    def _load_local_cache(self, interval: str) -> Dict[str, pd.DataFrame]:
        ...
        try:
            ...
            for sym in df.index.get_level_values("symbol").unique():
                ...
                if not sym_df.empty:
                    result[sym] = sym_df
            logger.info("Local bar cache loaded: %d tickers, path=%s", len(result), path)
            return result
        except Exception as exc:
            logger.warning("Local bar cache load failed: %s", exc)
            return {}

    def _save_local_cache(self, interval: str, bars: Dict[str, pd.DataFrame]) -> None:
        if not bars:
            return
        try:
            ...
            logger.info("Local bar cache saved: %d tickers, path=%s", len(bars), path)
        except Exception as exc:
            logger.warning("Local bar cache save failed: %s", exc)
    ...
    def get_bars(
        ...
        """Fetch OHLCV bars from yfinance with local parquet caching.

        On first run, fetches full history and saves to a local parquet cache.
        On subsequent runs, loads the cache from disk and fetches only recent days.
        """
        ...
        cached_bars = self._load_local_cache(interval)
        ...
        # Step 5: Save merged result to local cache
        if fetched:
            self._save_local_cache(interval, result)
