...
class Settings(BaseSettings):
    ...
    # -------------------------------------------------------------------------
    # LLM provider (provider-agnostic; any OpenAI-compatible endpoint)
    #
    # llm_base_url is empty by default; when set it routes to any
    # OpenAI-compatible server, e.g. OpenRouter (https://openrouter.ai/api/v1),
    # a LiteLLM proxy (http://localhost:4000/v1), LM Studio
    # (http://localhost:1234/v1), or vLLM (http://localhost:8000/v1).
    # -------------------------------------------------------------------------
    ...
    # DEPRECATED: extended_thinking_* are no-op compatibility fields. They are
    # retained so existing callers keep working, but they no longer drive any
    # behaviour. Do not rely on them for new code.
    extended_thinking_enabled: bool = False
    extended_thinking_budget: int = 2048          # LLM models: extended-thinking token budget
    extended_thinking_effort: str = "medium"       # LLM models: low | medium | high
