...
function getExecutionResult(
  ...
  _exitSignals: Record<string, unknown>[],
                ...
                <TableBody>
                  ...
                  {morningLlmRejected.map((r, i) => (
                    <TableRow key={`rej-${i}`}>
                      ...
                      <TableCell className="px-2">
                        <Badge className="text-[11px] bg-red-soft text-red">REJECT</Badge>
                      </TableCell>
                      ...
                      <TableCell className="text-xs font-mono px-2 text-red">rejected</TableCell>
                    </TableRow>
                  ))}
      ...
      {(eodDecisions.length > 0 || stopLossHits.length > 0) && (
        <section>
          <div className="flex items-center gap-2 mb-2">
            ...
            {Boolean(eodQuant?.regime) && <span className="text-xs text-muted-foreground">{String(eodQuant?.regime)}</span>}
          </div>
              ...
              <Table>
                ...
                <TableBody>
                  ...
                  {stopLossHits.map((e, i) => {
                    ...
                    return (
                    <TableRow key={`sl-${i}`} className="bg-red-soft/50">
                      <TableCell className="font-medium text-xs px-2"><TickerLink ticker={String(e.ticker)} date={date} /></TableCell>
                      <TableCell className="px-2">
                        <Badge className="text-[11px] bg-red text-white">STOP_EXIT</Badge>
                      </TableCell>
      ...
      <TableBody>
        {entries.map(([ticker, { data }]) => {
          ...
          return (
            <TableRow key={ticker}>
              <TableCell className="font-medium text-xs px-2">
                <TickerLink ticker={ticker} date={date} />{Boolean(data.veto_trade) && <span className="text-loss ml-1">VETO</span>}
              </TableCell>
    ...
    <div className="space-y-3">
      {quant && (
        <div className="flex flex-wrap gap-2">
            {Boolean(q?.regime) && <Badge variant="secondary" className="text-[10px]">Regime: {String(q?.regime)}</Badge>}
            {Boolean(q?.strategy) && <Badge variant="secondary" className="text-[10px]">{String(q?.strategy)}</Badge>}
...
function NotesPanel({ date: _date, dayDetail }: { date: string; dayDetail: CycleDetail | null }) {
      ...
      <CardHeader className="pb-2 cursor-pointer" onClick={() => setExpanded(!expanded)}>
        <CardTitle className="text-sm font-medium flex items-center gap-2 flex-wrap">
          ...
          {stopExitEvents.map((e, i) => (
            <Badge key={`se-${i}`} className="text-[10px] bg-red text-white">
...
export function CyclesTab({ sessionId, dateRange, dailyDates, isRunning }: Props) {
