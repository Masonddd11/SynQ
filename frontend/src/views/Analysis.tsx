...
function MetricBarChart({ label, entries, format, unit }: {
  label: string;
      ...
      <div style={{ height: entries.length * 28 + 20 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} layout="vertical" margin={{ left: 0, right: 40, top: 0, bottom: 0 }}>
            <XAxis type="number" tick={{ fontSize: 10, fill: 'var(--color-ink-soft)' }} hide />
            <YAxis type="category" dataKey="name" tick={{ fontSize: 10, fill: 'var(--color-ink-soft)' }} width={80} />
            <Tooltip
              contentStyle={{ background: 'var(--color-background)', border: '1px solid var(--color-hairline)', borderRadius: 0, fontSize: 11, fontFamily: 'var(--font-mono)' }}
              formatter={(value) => [`${format(Number(value))}${unit ?? ''}`, label]}
            />
            <Bar dataKey="value" radius={[0, 0, 0, 0]} isAnimationActive={false}
              label={{ position: 'right', fontSize: 10, formatter: (v) => format(Number(v)) }}
            >
              {data.map((d, i) => (
              ...
              <div className="h-[350px]">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={cumulativeData}>
                    ...
                    <XAxis dataKey="day" tick={{ fontSize: 11, fill: 'var(--color-ink-soft)' }} label={{ value: 'Day', position: 'insideBottomRight', offset: -5, fontSize: 11, fill: 'var(--color-ink-soft)' }} />
                    <YAxis tick={{ fontSize: 11, fill: 'var(--color-ink-soft)' }} />
                    <Tooltip
                      contentStyle={{ background: 'var(--color-background)', border: '1px solid var(--color-hairline)', borderRadius: 0, fontSize: 11, fontFamily: 'var(--font-mono)' }}
                      formatter={(value, name) => {
                        if (name === 'spy_return') return [`${Number(value).toFixed(2)}%`, 'SPY'];
                        const s = selected.find(s => String(name).startsWith(s.id));
                    ...
                    <Legend formatter={(value) => {
                      ...
                      const s = selected.find(s => String(value).startsWith(s.id));
              ...
              <div className="h-[250px]">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={cumulativeData}>
                    <CartesianGrid strokeDasharray="3 3" stroke="var(--color-hairline)" />
                    <XAxis dataKey="day" tick={{ fontSize: 11, fill: 'var(--color-ink-soft)' }} />
                    <YAxis tick={{ fontSize: 11, fill: 'var(--color-ink-soft)' }} />
                    <Tooltip
                      contentStyle={{ background: 'var(--color-background)', border: '1px solid var(--color-hairline)', borderRadius: 0, fontSize: 11, fontFamily: 'var(--font-mono)' }}
                      formatter={(value, name) => {
              ...
              <div className="h-[200px]">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={cumulativeData}>
                    <CartesianGrid strokeDasharray="3 3" stroke="var(--color-hairline)" />
                    <XAxis dataKey="day" tick={{ fontSize: 11, fill: 'var(--color-ink-soft)' }} />
                    <YAxis tick={{ fontSize: 11, fill: 'var(--color-ink-soft)' }} domain={[0, 100]} />
                    <Tooltip
                      contentStyle={{ background: 'var(--color-background)', border: '1px solid var(--color-hairline)', borderRadius: 0, fontSize: 11, fontFamily: 'var(--font-mono)' }}
                      formatter={(value, name) => {
                        const s = selected.find(s => String(name).startsWith(s.id));
                        return [`${Number(value).toFixed(1)}%`, s?.label ?? String(name)];
                      }}
                    />
                    {selected.map((s, i) => (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={cumulativeData}>
                    ...
                    <Tooltip
                      ...
                      formatter={(value, name) => {
                        const s = selected.find(s => String(name).startsWith(s.id));
                        return [String(Math.round(Number(value))), s?.label ?? String(name)];
                      }}
                    />
                    {selected.map((s, i) => (
                ...
                <div className="h-[250px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={monthlyData}>
                      <CartesianGrid strokeDasharray="3 3" stroke="var(--color-hairline)" />
                      <XAxis dataKey="month" tick={{ fontSize: 10, fill: 'var(--color-ink-soft)' }} />
                      <YAxis tick={{ fontSize: 11, fill: 'var(--color-ink-soft)' }} />
                      <Tooltip
                        contentStyle={{ background: 'var(--color-background)', border: '1px solid var(--color-hairline)', borderRadius: 0, fontSize: 11, fontFamily: 'var(--font-mono)' }}
                      formatter={(value, name) => {
                        const s = selected.find(s => s.id === String(name));
                        return [`${Number(value).toFixed(2)}%`, s?.label ?? String(name)];
                      }}
                      />
                      {selected.map((s, i) => (
                        <Bar key={s.id} dataKey={s.id} fill={LINE_COLORS[i]}
                          radius={[0, 0, 0, 0]} name={s.id} />
                      ))}
                      <ReferenceLine y={0} stroke="var(--color-ink-soft)" />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </CardContent>
