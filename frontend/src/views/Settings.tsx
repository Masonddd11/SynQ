...
function GeneralTab() {
  ...
  const isClaudeModel = model.model_id.toLowerCase().includes('claude');
  const isNovaModel = false;
        ...
        <CardContent className="space-y-3">
          ...
          {model.extended_thinking_enabled && !isClaudeModel && (
            <Field label="Reasoning Effort">
              ...
              <p className="text-[10px] text-muted-foreground mt-1">
                Reasoning effort for models that support it (OpenAI-compatible endpoints).
              </p>
            </Field>
          )}
        </CardContent>
  ...
  useEffect(() => {
    fetch('/api/settings/keys')
      ...
      .then((data: Record<string, unknown>) => {
        // The backend only returns stored keys — missing or null fields would
        // make inputs uncontrolled (value flips from undefined to a string on
        // first keystroke). Merge over EMPTY_KEYS so every field is ''.
        const normalized: ApiKeys = { ...EMPTY_KEYS };
        for (const field of Object.keys(EMPTY_KEYS) as (keyof ApiKeys)[]) {
