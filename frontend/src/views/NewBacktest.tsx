...
export function NewBacktestPage() {
  ...
  const [form, setForm] = useState<BacktestForm>(() => {
    const saved = typeof window !== 'undefined' ? localStorage.getItem('backtest_form') : null;
    if (saved) {
      try {
        ...
        <CardContent className="space-y-5">
                ...
                >
                  {filteredSnapshots.map((s) => (
                    <option key={s.session_id} value={s.session_id}>
                      {s.session_id} &mdash; ${s.portfolio_value.toLocaleString()}, {s.positions.length} pos, ends {s.final_date}
                    </option>
                  ))}
                </select>
              </Field>
          ...
          <div>
            ...
            >
              {submitting
                ? 'Starting...'
                : `${isResume ? 'Start from Snapshot' : 'Start Backtest'} (local)`}
            </button>
          </div>
        </CardContent>
      </Card>
