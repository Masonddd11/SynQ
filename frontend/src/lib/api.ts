...
export interface SystemServiceHealth {
  name: string;
  status: 'ok' | 'degraded' | 'down' | 'idle' | 'not_configured';
  detail: string;
}
...
export const api = {
  ...
  // System
  getSystemHealth: () => fetchJSON<SystemHealthResponse>('/system/health'),
  getSystemLog: (limit?: number) => fetchJSON<SystemLogResponse>('/system/log?limit=' + (limit ?? 50)),
  checkSystemService: (service: string) =>
    fetchJSON<SystemCheckResult>(`/system/check/${service}`, { method: 'POST' }),
};
