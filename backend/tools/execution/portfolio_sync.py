...
def sync_positions_from_alpaca(existing_positions=None) -> dict:
    ...
    Args:
        existing_positions: Optional dict of {symbol: Position} from caller
            (e.g. agent state loaded from the store). When provided, used instead of
            loading from local file — ensures metadata (strategy, entry_date,
            stop_loss) is preserved.
    ...
    try:
        ...
        portfolio.load()
        # If the local file is empty but the caller provided
        # positions from agent state, inject them so metadata is preserved.
